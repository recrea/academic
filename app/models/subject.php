...
class Subject extends AcademicModel {
	...
	var $validate = array(
		...
		function codeMustBeUnique($code){
			$course_id = $this->data[$this->alias];
			$subjects_with_same_code = $this->find('all', array('conditions' => array('Subject.code' => $code, 'Subject.course_id' => $course_id)));
			return count($subjects_with_same_code) == 0;
		}
}
